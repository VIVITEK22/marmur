...
if( $('.js-h-services-item').length == 3 ){
    $('.js-h-services-item').hover(function(){
...
//menu
$('body').append('<style type="text/css">.simple-compensate { margin-right: '+ getScrollbarWidth() +"px; }</style>")
var bodyMargin;
...
function getScrollbarWidth() {
    ...
    if($(window).innerHeight() == $('.main').innerHeight() + $('.footer').innerHeight()){
        scrollbarWidth = 0;
    }
...
$(window).on('load scroll', function(){
    ...
    if($(window).scrollTop() > 300 && scrollBottom < $('.main').innerHeight() - 200){
...
$(document).click( function(e){
    if ( !$('.js-project-list-wrap').is(e.target) && $('.js-project-list-wrap').has(e.target).length === 0
    && !$('.js-projects-open').is(e.target) && $('.js-projects-open').has(e.target).length === 0 ) {
...
//portfolio filter
$('.portfolio').css('min-height', $(window).innerHeight() - $('.post-title').innerHeight())
...
var bannerAutoplay;
var homeSliderLength = $('.home-banner__slide').length - $('.home-banner__slide.swiper-slide-duplicate').length;
...
$(document).on('click', '.home-banner__slide.swiper-slide-next, .js-h-banner-next', function(){
...
window.onload = function(){
	setTimeout(function(){
        if(!$('.preloader').hasClass('done')){
            ...
            $grid.isotope({ filter: $('.js-portfolio-filter.active').attr('data-filter') });
            setTimeout(function(){
                preloaderDone();
                $('body').removeClass('simple-compensate');
                setTimeout(function(){
...
$('.cards__list').each(function(i, el) {
    if ($(el).children('.cards__item--select').data('number') % 2 == 1) {
        $(el).children('.cards__item:nth-of-type(2n)').css('padding-top', '55px')
    } else {
        $(el).children('.cards__item:nth-of-type(2n+1)').css('padding-top', '55px')
    }
});
...
$('.result__image-line').draggable({
    ...
    drag: function(_, ui) {
        ...
        if (ui.position.left > 760) {
            ui.position.left = 760
        }
        $(this).closest('.result__image-section').children('.result__image.visual').css('width', `${ui.position.left}px`)
    }
})
...
$('.result__span-item').on('click', function() {
    if ($(this).hasClass('result__span--active')) {
        ...
        resultSwiper.slideTo(+index)
        $('.result__right--active').removeClass('result__right--active')
        $(`.result__right-body[data-number=${index}]`).addClass('result__right--active')
    }
})
